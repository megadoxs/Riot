...
export default function Clickable_Card({ element, prpimg, displayName, className, hasModal, modalListener}){

    const [isModalOpen, setIsModalOpen] = useState(false);

    useEffect(() => {
        if(modalListener)
            modalListener(isModalOpen)
    },[modalListener, isModalOpen])
    
    return(
        <div className = {`${className} ${styles.wrapper}`} onClick = {() => { if ((typeof hasModal === 'undefined' || hasModal === true) && !isModalOpen) setIsModalOpen(true); }}>
